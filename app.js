// ==================== OARCEL - COMPLETE WORKING VERSION ====================
...
function saveAllFiles() { try { const toSave = {}; for (const folder in allFiles) { toSave[folder] = allFiles[folder].map(file => ({ name: file.name, dataUrl: file.dataUrl, date: file.date })); } localStorage.setItem('oarcel_all_files', JSON.stringify(toSave)); return true; } catch(e) { return false; } }
function loadAllFiles() { try { const saved = localStorage.getItem('oarcel_all_files'); if (saved) { allFiles = JSON.parse(saved); let total = 0; for (const folder in allFiles) total += allFiles[folder].length; if (total > 0) showToast(`Loaded ${total} saved files`); } } catch(e) { allFiles = {}; } }
function getFilesForCurrentFolder() { const folderPath = currentPath.join("/"); return allFiles[folderPath] || []; }
async function addFileToCurrentFolder(file) { const folderPath = currentPath.join("/"); if (!allFiles[folderPath]) allFiles[folderPath] = []; const base64Data = await new Promise((resolve) => { const reader = new FileReader(); reader.onload = (e) => resolve(e.target.result); reader.readAsDataURL(file); }); allFiles[folderPath].push({ name: file.name, dataUrl: base64Data, date: new Date().toISOString() }); saveAllFiles(); return true; }
function deleteFileFromFolder(folderPath, fileName) { if (allFiles[folderPath]) { const index = allFiles[folderPath].findIndex(f => f.name === fileName); if (index !== -1) { allFiles[folderPath].splice(index, 1); if (allFiles[folderPath].length === 0) delete allFiles[folderPath]; saveAllFiles(); render(); showToast(`Deleted "${fileName}"`); return true; } } return false; }

// ==================== FOLDER MANAGEMENT ====================
function renameCurrentFolder() {
    if (currentPath.length === 0) { showToast("Cannot rename root folder", true); return; }
...
function addNewFolder() {
    ...
    if (folderName && folderName.trim() !== "") {
        ...
        else if (currentFolder && currentFolder[folderName]) showToast("❌ Folder already exists!", true);
    }
}
...
function deleteCurrentFolder() {
    if (currentPath.length === 0) { showToast("Cannot delete root folder", true); return; }
...
function openPDF(dataUrl) { viewer.classList.remove("hidden"); document.body.style.overflow = "hidden"; frame.src = "about:blank"; if (dataUrl.startsWith('data:application/pdf')) { fetch(dataUrl).then(res => res.blob()).then(blob => { const blobUrl = URL.createObjectURL(blob); frame.src = `https://docs.google.com/viewer?embedded=true&url=${encodeURIComponent(blobUrl)}`; }).catch(err => frame.src = dataUrl); return; } frame.src = `https://docs.google.com/viewer?embedded=true&url=${encodeURIComponent(dataUrl)}`; }
...
fileInput.addEventListener("change", async (e) => { const files_list = e.target.files; if (!files_list.length) return; let uploadedCount = 0; for (let file of files_list) { if (file.type === "application/pdf") { await addFileToCurrentFolder(file); uploadedCount++; } } if (uploadedCount > 0) { showToast(`${uploadedCount} PDF(s) saved permanently!`); render(); } fileInput.value = ""; });
...
function render() {
    ...
    // ACTION BUTTONS for folder management
    if (!isRoot) {
    ...
    // Show subfolders
    for (let key in folder) { content.appendChild(createCard(key, () => { currentPath.push(key); render(); }, true)); }
    ...
    if (isLeafFolder) {
        ...
        if (folderFiles.length === 0 && Object.keys(folder).length === 0) {
            ...
            folderFiles.forEach((f) => { content.appendChild(createCard(f.name, () => openPDF(f.dataUrl), false, true, folderPath, f.name)); });
        }
    } else { uploadBtn.classList.toggle("hidden", true); }
    updateStats();
}

// Make functions global
window.deleteFileFromFolder = (folderPath, fileName) => { if (confirm(`Delete "${fileName}"?`)) deleteFileFromFolder(folderPath, fileName); };
