...
function saveAllFilesToDB() {
    ...
    for (const folderPath in allFiles) {
        if (allFiles.hasOwnProperty(folderPath)) {
...
function createCard(title, onClick, isFolder = false, showDel = false, delPath = null, delName = null, showRename = false) {
    ...
    div.innerHTML = `
        ...
        <div class="card-buttons">
            ${showRename ? `<button class="rename-file-btn" data-rename='${JSON.stringify({ path: delPath, name: delName })}'><i class="fas fa-edit"></i> Rename</button>` : ''}
            ${showDel ? `<button class="delete-btn" data-del='${JSON.stringify({ path: delPath, name: delName })}'><i class="fas fa-trash"></i> Delete</button>` : ''}
        </div>
    `;
    div.onclick = onClick;
    const renameBtn = div.querySelector('.rename-file-btn');
    const delBtn = div.querySelector('.delete-btn');
    if (renameBtn) {
        renameBtn.onclick = (e) => {
            e.stopPropagation();
            const data = JSON.parse(renameBtn.dataset.rename);
            window.renameFile(data.path, data.name);
        };
    }
    if (delBtn) {
        delBtn.onclick = (e) => {
            e.stopPropagation();
            const data = JSON.parse(delBtn.dataset.del);
            window.deleteFile(data.path, data.name);
        };
    }
    return div;
}
...
function render() {
    ...
    bcDiv.innerHTML = `<div class="breadcrumb-item" data-bread="-1"><i class="fas fa-home"></i> Home</div>`;
    currentPath.forEach((f, i) => {
        bcDiv.innerHTML += `<span class="breadcrumb-separator">/</span><div class="breadcrumb-item" data-bread="${i}">${escapeHtml(f)}</div>`;
    });
    bcDiv.querySelectorAll('.breadcrumb-item').forEach(el => {
        const idx = parseInt(el.dataset.bread);
        el.onclick = () => navigateToBreadcrumb(idx);
    });
    ...
    if (currentPath.length === 0) {
        ...
        for (let dept in fileSystem) {
            ...
            html += `<div class="dept-card" data-dept="${dept}"><div class="dept-oval"><span>${dept}</span></div><div class="dept-arrow"><i class="fas fa-chevron-right"></i></div><div class="dept-info">${sub + fcount} items</div></div>`;
        }
        html += '</div>';
    ...
    if (!isRoot) {
        actionDiv.innerHTML = `<button class="action-btn" id="renameFolderBtn"><i class="fas fa-edit"></i> Rename Folder</button><button class="action-btn" id="deleteFolderBtn"><i class="fas fa-trash-alt"></i> Delete Folder</button><button class="action-btn" id="addSubfolderBtn"><i class="fas fa-plus"></i> Add Subfolder</button>`;
    } else {
        actionDiv.innerHTML = `<button class="action-btn" id="addDeptBtn"><i class="fas fa-building"></i> Add Department</button>`;
    }
    document.getElementById('content').appendChild(actionDiv);
    
    if (!isRoot) {
        document.getElementById('renameFolderBtn')?.addEventListener('click', renameCurrentFolder);
        document.getElementById('deleteFolderBtn')?.addEventListener('click', deleteCurrentFolder);
        document.getElementById('addSubfolderBtn')?.addEventListener('click', addNewFolder);
    } else {
        document.getElementById('addDeptBtn')?.addEventListener('click', addNewDepartment);
    }
    
    if (!isRoot && !isLeaf) {
        for (let key in folder) {
            document.getElementById('content').appendChild(createCard(key, () => {
...
function updateStats() {
    ...
    for (let k in allFiles) {
        if (allFiles[k]) fileCount += allFiles[k].length;
    }
...
function updateThemeIcon() {
    ...
    document.getElementById('themeToggle').innerHTML = isDark ? '<i class="fas fa-sun"></i><span>Light</span>' : '<i class="fas fa-moon"></i><span>Dark</span>';
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.renameFile = (p, old) => {
        ...
        if (nu?.trim()) renameFileInFolder(p, old, nu.trim());
    };
